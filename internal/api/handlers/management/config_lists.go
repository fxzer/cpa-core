...
func (h *Handler) PatchGeminiKey(c *gin.Context) {
	type geminiKeyPatch struct {
		APIKeyEntries  *[]config.ProviderAPIKeyEntry `json:"api-key-entries"`
	...
	if targetIndex == -1 && body.Match != nil {
		...
		if match != "" {
			for i := range h.cfg.GeminiKey {
				if providerConfigContainsAPIKey(h.cfg.GeminiKey[i].APIKeyEntries, match) {
					targetIndex = i
					break
				}
			}
	...
	if body.Value.APIKeyEntries != nil {
		entry.APIKeyEntries = config.NormalizeProviderAPIKeyEntries(*body.Value.APIKeyEntries)
	...
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		if baseRaw, okBase := c.GetQuery("base-url"); okBase {
			...
			for _, v := range h.cfg.GeminiKey {
				if providerConfigContainsAPIKey(v.APIKeyEntries, val) && strings.TrimSpace(v.BaseURL) == base {
					continue
				}
				out = append(out, v)
			}
		...
		matchCount := 0
		for i := range h.cfg.GeminiKey {
			if providerConfigContainsAPIKey(h.cfg.GeminiKey[i].APIKeyEntries, val) {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
...
func (h *Handler) PatchClaudeKey(c *gin.Context) {
	...
	if targetIndex == -1 && body.Match != nil {
		...
		for i := range h.cfg.ClaudeKey {
			if providerConfigContainsAPIKey(h.cfg.ClaudeKey[i].APIKeyEntries, match) {
				targetIndex = i
				break
			}
		}
...
func (h *Handler) DeleteClaudeKey(c *gin.Context) {
	...
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		...
		matchCount := 0
		for i := range h.cfg.ClaudeKey {
			if providerConfigContainsAPIKey(h.cfg.ClaudeKey[i].APIKeyEntries, val) {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
...
func (h *Handler) PutVertexCompatKeys(c *gin.Context) {
	...
	for i := range arr {
		...
		if len(arr[i].APIKeyEntries) == 0 {
			c.JSON(400, gin.H{"error": fmt.Sprintf("vertex-api-key[%d].api-key-entries is required", i)})
			return
		}
	}
	h.mu.Lock()
	...
	if targetIndex == -1 && body.Match != nil {
		...
		if match != "" {
			for i := range h.cfg.VertexCompatAPIKey {
				if providerConfigContainsAPIKey(h.cfg.VertexCompatAPIKey[i].APIKeyEntries, match) {
					targetIndex = i
					break
				}
			}
...
func (h *Handler) DeleteVertexCompatKey(c *gin.Context) {
	...
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		...
		matchCount := 0
		for i := range h.cfg.VertexCompatAPIKey {
			if providerConfigContainsAPIKey(h.cfg.VertexCompatAPIKey[i].APIKeyEntries, val) {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
...
// oauth-excluded-models: map[string]map[string]bool
func (h *Handler) GetOAuthExcludedModels(c *gin.Context) {
	c.JSON(200, gin.H{"oauth-excluded-models": config.NormalizeOAuthExcludedModelsConfig(h.cfg.OAuthExcludedModels)})
}
...
func (h *Handler) PutOAuthExcludedModels(c *gin.Context) {
	...
	var entries config.OAuthExcludedModelsConfig
	if err = json.Unmarshal(data, &entries); err != nil {
		var wrapper struct {
			Items config.OAuthExcludedModelsConfig `json:"items"`
		}
		if err2 := json.Unmarshal(data, &wrapper); err2 != nil {
			var legacy map[string][]string
			if err3 := json.Unmarshal(data, &legacy); err3 != nil {
				var legacyWrapper struct {
					Items map[string][]string `json:"items"`
				}
				if err4 := json.Unmarshal(data, &legacyWrapper); err4 != nil {
					c.JSON(400, gin.H{"error": "invalid body"})
					return
				}
				legacy = legacyWrapper.Items
			}
			entries = config.OAuthExcludedModelsConfigFromLegacy(legacy)
		} else {
			entries = wrapper.Items
		}
	}
	h.cfg.OAuthExcludedModels = config.NormalizeOAuthExcludedModelsConfig(entries)
	h.persist(c)
}
...
func (h *Handler) PatchOAuthExcludedModels(c *gin.Context) {
	...
	providerModels := parseOAuthExcludedProviderPatch(body.Models)
	...
	h.cfg.OAuthExcludedModels[provider] = providerModels
	h.persist(c)
}

func parseOAuthExcludedProviderPatch(raw json.RawMessage) config.OAuthExcludedProviderModels {
	if len(raw) == 0 || string(raw) == "null" {
		return nil
	}
	var asMap map[string]bool
	if err := json.Unmarshal(raw, &asMap); err == nil {
		out := make(config.OAuthExcludedProviderModels, len(asMap))
		for name, disabled := range asMap {
			if disabled {
	...
	if err := json.Unmarshal(raw, &asList); err == nil {
		converted := config.OAuthExcludedModelsConfigFromLegacy(map[string][]string{
			"provider": config.NormalizeExcludedModels(asList),
		})
		if converted == nil {
			return nil
		}
...
func (h *Handler) PatchOAuthModelAlias(c *gin.Context) {
	...
	if len(normalized) == 0 {
		if len(body.Aliases) > 0 {
			c.JSON(400, gin.H{"error": "no valid alias mappings: name and alias must differ and be non-empty"})
			return
		}
...
func (h *Handler) PatchCodexKey(c *gin.Context) {
	...
	if targetIndex == -1 && body.Match != nil {
		...
		for i := range h.cfg.CodexKey {
			if providerConfigContainsAPIKey(h.cfg.CodexKey[i].APIKeyEntries, match) {
				targetIndex = i
				break
			}
		}
...
func (h *Handler) DeleteCodexKey(c *gin.Context) {
	...
	if val := strings.TrimSpace(c.Query("api-key")); val != "" {
		...
		matchCount := 0
		for i := range h.cfg.CodexKey {
			if providerConfigContainsAPIKey(h.cfg.CodexKey[i].APIKeyEntries, val) {
				matchCount++
				if matchIndex == -1 {
					matchIndex = i
				}
...
func normalizeOpenAICompatibilityEntry(entry *config.OpenAICompatibility) {
	...
	for i := range entry.APIKeyEntries {
		...
		entry.APIKeyEntries[i].Remark = strings.TrimSpace(entry.APIKeyEntries[i].Remark)
		if trimmed != "" {
...
func providerConfigContainsAPIKey(entries []config.ProviderAPIKeyEntry, apiKey string) bool {
	return config.ProviderAPIKeyEntriesContain(entries, apiKey)
}
...
func normalizeClaudeKey(entry *config.ClaudeKey) {
	...
	entry.BaseURL = strings.TrimSpace(entry.BaseURL)
	entry.APIKeyEntries = config.NormalizeProviderAPIKeyEntries(entry.APIKeyEntries)
	entry.Headers = config.NormalizeHeaders(entry.Headers)
