...
type Config struct {
	...
	// UsageStatisticsEnabled toggles whether request usage events are recorded.
	UsageStatisticsEnabled bool `yaml:"usage-statistics-enabled" json:"usage-statistics-enabled"`

	// Usage configures SQLite-backed request event persistence (enabled by default).
	...
	// Each provider maps model IDs/patterns to disabled=true. Absent keys mean enabled.
	OAuthExcludedModels OAuthExcludedModelsConfig `yaml:"oauth-excluded-models,omitempty" json:"oauth-excluded-models,omitempty"`
...
// Repeating the same Alias with different Name values builds an alias pool; requests
// to that alias round-robin across the upstream names within the same auth channel.
...
// ClaudeKey represents the configuration for a Claude API credential group.
type ClaudeKey struct {
	// APIKeyEntries lists API keys that share the settings below.
	APIKeyEntries []ProviderAPIKeyEntry `yaml:"api-key-entries" json:"api-key-entries"`

	// Priority controls selection preference when multiple credentials match.
	// Higher values are preferred; defaults to 0.
	Priority int `yaml:"priority,omitempty" json:"priority,omitempty"`

	// Name is an optional human-readable label for this credential group.
...
// CodexKey represents the configuration for a Codex API credential group.
...
// GeminiKey represents the configuration for a Gemini API credential group.
...
// OpenAICompatibilityAPIKey is an alias for ProviderAPIKeyEntry.
type OpenAICompatibilityAPIKey = ProviderAPIKeyEntry
...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	cfg.OAuthExcludedModels = NormalizeOAuthExcludedModelsConfig(cfg.OAuthExcludedModels)
...
// allows multiple aliases per upstream name, and allows multiple upstream names per alias
// to form alias pools. Duplicate name/alias pairs within a channel are removed.
...
func (cfg *Config) SanitizeCodexKeys() {
	...
	for i := range cfg.CodexKey {
		...
		e.APIKeyEntries = NormalizeProviderAPIKeyEntries(e.APIKeyEntries)
		if e.BaseURL == "" || len(e.APIKeyEntries) == 0 {
			continue
		}
		out = append(out, e)
	}
...
func (cfg *Config) SanitizeGeminiKeys() {
	...
	for i := range cfg.GeminiKey {
		...
		uniqueKey := entry.BaseURL + "|" + entry.Prefix + "|" + FirstProviderAPIKey(entry.APIKeyEntries)
...
// ProviderDisplayName returns the configured human-readable label for a provider key group.
// Prefix takes precedence over Name when both are set.
func ProviderDisplayName(prefix, name string) string {
	if label := normalizeModelPrefix(prefix); label != "" {
		return label
	}
...
// NormalizeOAuthExcludedModels is kept for legacy callers that still pass []string maps.
func NormalizeOAuthExcludedModels(entries map[string][]string) OAuthExcludedModelsConfig {
	return NormalizeOAuthExcludedModelsConfig(OAuthExcludedModelsConfigFromLegacy(entries))
}
...
func (cfg *Config) migrateLegacyGeminiKeys(legacy []string) bool {
	...
	seen := make(map[string]struct{}, len(cfg.GeminiKey))
	for i := range cfg.GeminiKey {
		key := FirstProviderAPIKey(cfg.GeminiKey[i].APIKeyEntries)
		if key == "" {
			continue
		}
		seen[key] = struct{}{}
	...
	for _, raw := range legacy {
		...
		cfg.GeminiKey = append(cfg.GeminiKey, GeminiKey{
			APIKeyEntries: []ProviderAPIKeyEntry{{APIKey: key}},
		})
		seen[key] = struct{}{}
		changed = true
	}
