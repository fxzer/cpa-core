...
func TestSanitizeOAuthModelAlias_AllowsMultipleNamesForSameAlias(t *testing.T) {
	cfg := &Config{
		OAuthModelAlias: map[string][]OAuthModelAlias{
			"codex": {
				...
				{Name: "gemini-3.1-flash-lite", Alias: "mini", Fork: true},
				{Name: "gpt-5.4-mini", Alias: "mini", Fork: true},
			},
		},
	}
	...
	if aliases[0].Name != "gpt-5.4-mini" || aliases[0].Alias != "mini" {
		t.Fatalf("unexpected first alias: %+v", aliases[0])
	}
	if aliases[1].Name != "gemini-3.1-flash-lite" || aliases[1].Alias != "mini" {
