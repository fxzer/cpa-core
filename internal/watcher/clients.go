...
func BuildAPIKeyClients(cfg *config.Config) (int, int, int, int, int) {
	...
	for i := range cfg.GeminiKey {
		geminiAPIKeyCount += len(cfg.GeminiKey[i].APIKeyEntries)
	}
	for i := range cfg.VertexCompatAPIKey {
		vertexCompatAPIKeyCount += len(cfg.VertexCompatAPIKey[i].APIKeyEntries)
	}
	for i := range cfg.ClaudeKey {
		claudeAPIKeyCount += len(cfg.ClaudeKey[i].APIKeyEntries)
	}
	for i := range cfg.CodexKey {
		codexAPIKeyCount += len(cfg.CodexKey[i].APIKeyEntries)
	}
