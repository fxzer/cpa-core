...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if len(oldCfg.GeminiKey) != len(newCfg.GeminiKey) {
		...
		for i := range oldCfg.GeminiKey {
			...
			if strings.TrimSpace(o.Name) != strings.TrimSpace(n.Name) {
				changes = append(changes, fmt.Sprintf("gemini[%d].name: %s -> %s", i, strings.TrimSpace(o.Name), strings.TrimSpace(n.Name)))
			}
			if !config.ProviderAPIKeyEntriesEqual(o.APIKeyEntries, n.APIKeyEntries) {
				changes = append(changes, fmt.Sprintf("gemini[%d].api-key-entries: updated (%d -> %d entries)", i, len(o.APIKeyEntries), len(n.APIKeyEntries)))
			}
	...
	if len(oldCfg.ClaudeKey) != len(newCfg.ClaudeKey) {
		...
		for i := range oldCfg.ClaudeKey {
			...
			if strings.TrimSpace(o.Name) != strings.TrimSpace(n.Name) {
				changes = append(changes, fmt.Sprintf("claude[%d].name: %s -> %s", i, strings.TrimSpace(o.Name), strings.TrimSpace(n.Name)))
			}
			if !config.ProviderAPIKeyEntriesEqual(o.APIKeyEntries, n.APIKeyEntries) {
				changes = append(changes, fmt.Sprintf("claude[%d].api-key-entries: updated (%d -> %d entries)", i, len(o.APIKeyEntries), len(n.APIKeyEntries)))
			}
			if !equalStringMap(o.Headers, n.Headers) {
	...
	if len(oldCfg.CodexKey) != len(newCfg.CodexKey) {
		...
		for i := range oldCfg.CodexKey {
			...
			if strings.TrimSpace(o.Name) != strings.TrimSpace(n.Name) {
				changes = append(changes, fmt.Sprintf("codex[%d].name: %s -> %s", i, strings.TrimSpace(o.Name), strings.TrimSpace(n.Name)))
			}
			if o.Websockets != n.Websockets {
				changes = append(changes, fmt.Sprintf("codex[%d].websockets: %t -> %t", i, o.Websockets, n.Websockets))
			}
			if !config.ProviderAPIKeyEntriesEqual(o.APIKeyEntries, n.APIKeyEntries) {
				changes = append(changes, fmt.Sprintf("codex[%d].api-key-entries: updated (%d -> %d entries)", i, len(o.APIKeyEntries), len(n.APIKeyEntries)))
			}
			if !equalStringMap(o.Headers, n.Headers) {
	...
	if len(oldCfg.VertexCompatAPIKey) != len(newCfg.VertexCompatAPIKey) {
		...
		for i := range oldCfg.VertexCompatAPIKey {
			...
			if strings.TrimSpace(o.Name) != strings.TrimSpace(n.Name) {
				changes = append(changes, fmt.Sprintf("vertex[%d].name: %s -> %s", i, strings.TrimSpace(o.Name), strings.TrimSpace(n.Name)))
			}
			if !config.ProviderAPIKeyEntriesEqual(o.APIKeyEntries, n.APIKeyEntries) {
				changes = append(changes, fmt.Sprintf("vertex[%d].api-key-entries: updated (%d -> %d entries)", i, len(o.APIKeyEntries), len(n.APIKeyEntries)))
			}
