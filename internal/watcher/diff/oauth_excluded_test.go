...
func TestDiffOAuthExcludedModelChanges(t *testing.T) {
	oldMap := config.OAuthExcludedModelsConfigFromLegacy(map[string][]string{
		"ProviderA": {"model-1", "model-2"},
		"providerB": {"x"},
	})
	newMap := config.OAuthExcludedModelsConfigFromLegacy(map[string][]string{
...
func TestSummarizeOAuthExcludedModels_NormalizesKeys(t *testing.T) {
	out := SummarizeOAuthExcludedModels(config.OAuthExcludedModelsConfigFromLegacy(map[string][]string{
		"ProvA": {"X"},
		"":      {"ignored"},
	}))
	if len(out) != 1 {
