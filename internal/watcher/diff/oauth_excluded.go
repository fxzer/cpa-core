...
func SummarizeOAuthExcludedModels(entries config.OAuthExcludedModelsConfig) map[string]ExcludedModelsSummary {
	if len(entries) == 0 {
		return nil
	}
	...
	for k := range entries {
		...
		out[key] = SummarizeExcludedModels(entries.Patterns(key))
	}
	return out
}

// DiffOAuthExcludedModelChanges compares OAuth excluded models maps.
func DiffOAuthExcludedModelChanges(oldMap, newMap config.OAuthExcludedModelsConfig) ([]string, []string) {
